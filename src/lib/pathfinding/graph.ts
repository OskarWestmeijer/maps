 ...
 * Each node has between 1–3 random neighbors with random edge weights (1–10).
 */
export function generateGraph(nodeCount: number): Graph {
	console.log('Generating graph: ' + nodeCount);
	if (nodeCount < 1) {
	...
	const nodes: Node[] = Array.from({ length: nodeCount }, (_, i) => ({
		id: String.fromCharCode(65 + i), // 65 = 'A'
		neighbors: [],
		visited: false,
		distance: Infinity
	}));
	...
	for (const node of nodes) {
		...
		for (const neighbor of chosenNeighbors) {
			const edgeWeight = Math.floor(Math.random() * 10) + 1; // edge weights 1–10
