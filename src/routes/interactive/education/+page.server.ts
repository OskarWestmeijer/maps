...
import { EMPTY_EDUCATION_STATS } from '$lib/interactive/education';
...
// under the panel heading, as all four other loaders do it: no PxWeb export carries a membership
...
export const load: PageServerLoad = () => {
	...
	return {
		finland: loadGeometry(kunnatCollection, { emptyStats: EMPTY_EDUCATION_STATS }),
		maakunta: loadGeometry(maakunnatCollection, { emptyStats: EMPTY_EDUCATION_STATS }),
		tampere: loadGeometry(JSON.parse(tampereGeojson) as KuntaCollection, {
			emptyStats: EMPTY_EDUCATION_STATS,
			isSubset: true,
