...
/**
 * The composite score. Unlike the other two maps nothing here is a published figure — every
 * number is derived from the two exports at page load — so these assertions double as the
...
test('every municipality is drawn and the ends of the ranking are offered', async ({ page }) => {
	...
	await expect(panel.getByRole('button', { name: /^5 Vårdö/ })).toBeVisible();
	await expect(panel.getByRole('button', { name: /^300 Kannonkoski/ })).toBeVisible();

	// Each row names the municipality's maakunta, which is most of what the two ends have to
	// say — the top is Ahvenanmaa and Pohjanmaa, the bottom is Karjala.
...
test('the panel shows the score, its rank, and the figures behind it', async ({ page }) => {
	...
	await expect(panel.getByText('rank 29 of 304')).toBeVisible();
...
test('municipalities are coloured by where their score sits, diverging around 50', async ({
	...
	// deepest red, and Tampere (49,5) the neutral middle.
...
test("a municipality's score does not change when the Tampere tab is opened", async ({ page }) => {
	...
	await expect(panel.getByText('rank 107 of 304')).toBeVisible();
